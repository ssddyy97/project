document.addEventListener('DOMContentLoaded', () => {
    ...
    if (likeButton && likeCountSpan) {
        likeButton.addEventListener('click', async () => {
            const episodeId = likeButton.dataset.episodeId;
            if (!episodeId) {
                console.error('Episode ID not found on like button.');
                return;
            }

            try {
                const response = await fetch(`${window.location.origin}/podcast/${episodeId}/like`, {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    console.error('Error liking podcast:', response.status, errorText);
